...
function CheckoutReturn() {
  ...
  const publicUrl = typeof window !== "undefined" ? `${window.location.origin}/para/${slug ?? ""}` : "";
    ...
    (async () => {
      try {
        ...
        if (res.paymentStatus === "paid" || res.status === "complete") {
          setState("paid");
          // Publish locally: mark slug as paid + plan + (expiry for temporary)
          if (slug) {
            const expiresAt = plan === "eternal"
              ? null
              : new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString();
            const published = JSON.parse(localStorage.getItem("memora:published") ?? "{}");
            published[slug] = {
              plan: plan ?? "temporary",
            ...
            localStorage.setItem("memora:published", JSON.stringify(published));
          }
        } else {
          setState("open");
        }
  ...
  const share = async () => {
    if (navigator.share) {
      try { await navigator.share({ title: "Sua homenagem Memora", url: publicUrl }); } catch {}
    } else {
      copy();
    }
  };
    ...
    return (
      <main className="bg-builder grid min-h-screen place-items-center px-6 text-center">
        <div className="max-w-md">
          ...
          <p className="mt-2 text-sm text-muted-foreground">A homenagem só publica após a confirmação. Tente novamente.</p>
          <Link to="/checkout" className="btn-gold mt-6 inline-flex text-sm">Voltar ao checkout</Link>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="bg-builder min-h-screen px-6 py-12">
      <div className="mx-auto max-w-2xl text-center">
        ...
        <p className="mt-4 font-mono text-[10px] uppercase tracking-[0.25em] text-primary/80">Pagamento confirmado</p>
        <h1 className="mt-2 font-display text-4xl font-extrabold sm:text-5xl">Sua homenagem está no ar ✨</h1>
        <p className="mt-3 text-sm text-muted-foreground">
          {email && <>Mandamos o link para <span className="text-cream">{email}</span>. </>}
          {plan === "eternal" ? "Link vitalício, fica para sempre." : "Link válido por 1 ano — avisaremos 30 dias antes de expirar."}
        </p>

        <div className="mt-8 grid gap-6 sm:grid-cols-[auto_1fr] sm:text-left">
          <div className="mx-auto rounded-2xl bg-cream p-4">
          ...
          <div className="flex flex-col justify-center gap-3">
            ...
            <p className="break-all rounded-xl border border-primary/40 bg-primary/10 px-3 py-2 font-mono text-sm text-primary">{publicUrl}</p>
            <div className="flex flex-wrap gap-2">
              ...
              <Link to="/para/$slug" params={{ slug: slug ?? "" }} className="btn-gold text-xs">
                Abrir homenagem
              </Link>
            </div>
          </div>
        ...
        <div className="mt-10 flex justify-center gap-2 text-sm">
          <Link to="/" className="btn-ghost text-xs">Voltar ao início</Link>
        </div>
      </div>
    </main>
  );
